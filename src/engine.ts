...
export class LcmContextEngine implements ContextEngine {
  ...
  private async resolveSummarize(params: {
    ...
    summarize: (text: string, aggressive?: boolean, options?: LcmSummarizeOptions) => Promise<string>;
    summaryModel: string;
  }> {
  ...
  private resolveBeaconMetadata(params: {
    ...
    const sessionKeyForParsing = params.sessionKey ?? safeString(lp.sessionKey);
    const parsedFromKey = sessionKeyForParsing
      ? this.deps.parseAgentSessionKey(sessionKeyForParsing)
      : null;
  ...
  async afterTurn(params: {
    ...
    if (this.config.crossSession.enabled === true) {
      ...
      const sessionKey = params.sessionKey ?? safeString(legacyParams?.sessionKey) ?? params.sessionId;
