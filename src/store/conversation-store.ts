...
const CONV_COLS = "conversation_id, session_id, session_key, active, archived_at, title, bootstrapped_at, created_at, updated_at";
...
export class ConversationStore {
  ...
  async createConversation(input: CreateConversationInput): Promise<ConversationRecord> {
    ...
    const result = await this.db.run(
      `INSERT INTO conversations (session_id, session_key, active, archived_at, title)
       VALUES (${d.p()}, ${d.p()}, ${d.p()}, ${d.p()}, ${d.p()}) RETURNING conversation_id`,
      [
        input.sessionId,
    ...
    if (existing) {
      ...
      if (existing.active && !existing.sessionKey) {
        ...
        await this.db.run(
          ...
          [normalizedSessionKey, existing.conversationId],
        );
  ...
  async archiveConversation(conversationId: ConversationId): Promise<void> {
    ...
    await this.db.run(
      `UPDATE conversations
       SET active = 0,
           archived_at = COALESCE(archived_at, ${d.now()}),
           updated_at = ${d.now()}
       WHERE conversation_id = ${d.p()}`,
      [conversationId],
    );
  ...
  private async searchLike(
    ...
    return result.rows
      .map((row) => {
        // Post-filter: re-check against normalized content so that stripped
        // helper text (e.g. "Use lcm_describe ...") is excluded even when
        // the raw LIKE matched the un-normalized content column.
