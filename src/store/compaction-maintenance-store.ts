...
export type ConversationCompactionMaintenanceRecord = {
  ...
  /** Threshold that triggered the recorded debt; drains reuse it for consistency. */
  ...
  async requestProactiveCompactionDebt(input: {
    ...
    await this.saveConversationCompactionMaintenance(
      mergeMaintenanceRecord(input.conversationId, existing, {
        ...
        // Unlike the diagnostics above, the threshold is not inherited from a
        // previous debt: it is only meaningful for the decision that recorded
        // this debt, so re-recording without one resets it to null.
