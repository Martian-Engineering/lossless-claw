...
function resolveFocusTargetTokens(summaryTokens: number): number {
  ...
  return Math.max(1000, Math.min(MAX_FOCUS_BRIEF_TARGET_TOKENS, Math.floor(summaryTokens)));
}
...
function resolveFocusExpansionTokenCap(params: {
  ...
  const summaryDerived = Math.max(configured, Math.floor(params.summaryTokens * 2));
  return Math.min(60_000, Math.max(configured, summaryDerived, 20_000));
}
...
function buildFocusBriefTask(params: {
  ...
  return [
    ...
    "",
    "You are a delegated subagent. Your job is to build a rich, evidence-oriented brief that a future model can use as task-shaped memory.",
    "",
    "Focus prompt:",
    `<focus_prompt>${params.focusPrompt}</focus_prompt>`,
    "",
    `Conversation ID: ${params.conversationId}`,
    `Target brief length: up to ${params.targetTokens} tokens`,
    ...
    "3. Use lcm_expand directly on high-value summary IDs to recover key details. You are in delegated context, so do NOT call lcm_expand_query.",
