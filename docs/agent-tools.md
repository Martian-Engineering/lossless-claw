...
- `full_text` — FTS5 keyword search. Defaults to AND matching; quote phrases. Not a regex — alternation (`A|B`), wildcards (`.*`), character classes (`[abc]`), and anchors (`^foo`, `foo$`) require `mode: "regex"`. Keep the default `sort: "recency"` for recent events, `sort: "relevance"` for the best older topic match, `sort: "hybrid"` for relevance without giving up recency.
