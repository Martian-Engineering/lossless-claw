...
Lossless-claw requires OpenClaw `2026.6.10` or newer so the host can provide the
SQLite transcript runtime APIs used during session bootstrap and enforce
context-engine runtime capabilities before an agent run starts. Agent runs need a
native host that provides the full context-engine lifecycle: session bootstrap,
...
use a generic CLI harness, use an older compatible `lossless-claw` release pinned
to your OpenClaw version.
