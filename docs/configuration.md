...
| `backgroundSummaryPreparationEnabled` | `boolean` | `false` | `LCM_BACKGROUND_SUMMARY_PREPARATION_ENABLED` | Enables background preparation of pending summary batches. Prepared leaves can be condensed further at arbitrary depths while hidden; prepared summaries are not assembled, searched, used for transcript GC, or treated as canonical until threshold compaction validates and publishes a complete batch atomically. If validation fails, Lossless supersedes the batch and falls back to the normal full-sweep path. |
