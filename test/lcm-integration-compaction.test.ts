...
describe("LCM integration: compaction", () => {
  ...
  it("evaluate accepts a per-call contextThreshold override", async () => {
    ...
    // Out-of-range overrides are ignored in favor of the configured threshold.
    const invalidOverrideDecision = await compactionEngine.evaluate(CONV_ID, 600, undefined, {
      contextThreshold: 1.5,
    });
    expect(invalidOverrideDecision).toMatchObject({
      shouldCompact: false,
      threshold: 450,
    });
  });
