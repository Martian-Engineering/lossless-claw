...
describe("LcmContextEngine afterTurn", () => {
  ...
  it("afterTurn does not use tokenBudget as a model context-window override fallback", async () => {
    ...
    // The 200k budget must not satisfy modelContextWindowMax — the rule does
    // not match, so the global threshold is the one passed through.
    expect(evaluateSpy).toHaveBeenCalledWith(expect.any(Number), 200_000, 50_000, {
      contextThreshold: getEngineConfig(engine).contextThreshold,
    });
  });

  it("afterTurn matches window override rules on legacy model context-window metadata", async () => {
    const engine = createEngineWithConfig({
      contextThresholdOverrides: [
        {
          match: { modelContextWindowMax: 250_000 },
