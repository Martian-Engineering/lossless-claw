...
describe("assemble emergency drain force=true", () => {
  it("calls compact despite backoff when force=true via assemble emergency drain", async () => {
    vi.useFakeTimers({ toFake: ["Date"] });
    vi.setSystemTime(new Date("2026-07-14T12:00:00.000Z"));
    try {
      const engine = createEngineWithConfig({ freshTailCount: 1 });
      const sessionId = "emergency-drain-force-true";
      ...
      // Set up maintenance with pending debt and active backoff
      ...
      // Spy on executeCompactionCore — the real method called after force=true
      // bypasses the backoff. Note: this is invoked via this.compaction.compact(),
      // not engine.compact(), so we spy at the executeCompactionCore level.
      const privateEngine = engine as unknown as {
      ...
      // executeCompactionCore should have been called — force=true bypassed backoff
  ...
  it("without force, backoff blocks compaction during normal drain", async () => {
    vi.useFakeTimers({ toFake: ["Date"] });
    vi.setSystemTime(new Date("2026-07-14T12:00:00.000Z"));
    try {
      const engine = createEngineWithConfig({ freshTailCount: 1 });
      const sessionId = "normal-drain-backoff-blocks";
      const tokenBudget = 1000;
      ...
      const result = await privateEngine.consumeDeferredCompactionDebt({
        conversationId: conversation!.conversationId,
        sessionId,
        tokenBudget,
      });

      // Without force, backoff should block — executeCompactionCore not called
